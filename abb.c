 ...
 * Pre: arbol fue previamente creado.
 */
abb_nodo_t * abb_obtener_nodo_con_padre(const abb_t * arbol, const abb_nodo_t * padre, const char * clave);
 ...
 */
abb_nodo_t  * crear_abb_nodo(const char * clave, void * dato, abb_nodo_t * izq, abb_nodo_t * der);
...
void* abb_borrar_nodo_con_hijo_unico(abb_t *arbol,abb_nodo_t* nodo,abb_nodo_t* padre, abb_nodo_t* nodo_reemplazante);
...
void* abb_borrar(abb_t *arbol, const char *clave) {
	...
	if(!padre) nodo = arbol->raiz;
	else nodo = abb_obtener_nodo_con_padre(arbol, padre, clave);
	
	arbol->cantidad--;

	if(!arbol->cantidad) arbol->raiz = NULL;
...
void *abb_obtener(const abb_t *arbol, const char *clave) {
	abb_nodo_t * candidato_padre = abb_obtener_candidato_padre(arbol, clave);
	if(!candidato_padre) return NULL;
...
bool abb_pertenece(const abb_t *arbol, const char *clave) {
	abb_nodo_t * candidato_padre = abb_obtener_candidato_padre(arbol, clave);
	if(!candidato_padre) return false;
...
bool _abb_in_order(abb_nodo_t *nodo, bool visitar(const char *, void *, void *), void *extra) {
	...
	if(iterar) _abb_in_order(nodo->der, visitar, extra);
	return true;
...
abb_nodo_t* _abb_obtener_padre(abb_nodo_t* nodo, const char * clave, abb_comparar_clave_t cmp, abb_nodo_t* padre) {
	if(!nodo) return NULL;
	
	if(cmp(clave, nodo->clave) == 0) return padre;
...
abb_nodo_t * _abb_obtener_candidato_padre(abb_nodo_t * nodo, const char * clave, abb_comparar_clave_t cmp) {
	if(!nodo) return NULL;
...
void* abb_borrar_hoja(abb_nodo_t* nodo,abb_nodo_t* padre) {
	if(!padre) return _abb_nodo_borrar(nodo);
...
void* abb_borrar_nodo_con_hijo_unico(abb_t *arbol,abb_nodo_t* nodo,abb_nodo_t* padre,abb_nodo_t* nodo_reemplazante) {
	...
	else padre->der = nodo_reemplazante;
...
void* abb_borrar_nodo_con_dos_hijos(abb_nodo_t* nodo) {
	...
	if(reemplazante->der) 
		return abb_borrar_nodo_con_hijo_unico(NULL, reemplazante, padre_reemplazante, reemplazante->der);
